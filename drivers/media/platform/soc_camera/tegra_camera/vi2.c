				    ...
				    struct soc_camera_device *icd)
{
	...
	TC_VI_REG_WT(cam, TEGRA_CSI_DEBUG_CONTROL,
			0x3 | (0x1 << 5) | (0x40 << 8));
#endif
	TC_VI_REG_WT(cam, TEGRA_CSI_PHY_CILA_CONTROL0, 0x6);
	TC_VI_REG_WT(cam, TEGRA_CSI_PHY_CILB_CONTROL0, 0x6);
				     ...
				     struct soc_camera_device *icd)
{
	...
	if (pdata->port == TEGRA_CAMERA_PORT_CSI_B) {
		TC_VI_REG_WT(cam, TEGRA_CSI_PHY_CILC_CONTROL0, 0x6);
		TC_VI_REG_WT(cam, TEGRA_CSI_PHY_CILD_CONTROL0, 0x6);
	} else if (pdata->port == TEGRA_CAMERA_PORT_CSI_C)
		TC_VI_REG_WT(cam, TEGRA_CSI_PHY_CILE_CONTROL0, 0x6);
