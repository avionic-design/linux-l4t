...
static void tegra_camera_capture_setup(struct tegra_camera_dev *pcdev)
{
	...
	if (port == TEGRA_CAMERA_PORT_CSI_A) {
		...
		/* CSI A */
		TC_VI_REG_WT(pcdev, TEGRA_CSI_VI_INPUT_STREAM_CONTROL,
			     0x00000000);
		TC_VI_REG_WT(pcdev, TEGRA_CSI_HOST_INPUT_STREAM_CONTROL,
			     0x00000000);
		...
		TC_VI_REG_WT(pcdev, TEGRA_CSI_PIXEL_STREAM_A_WORD_COUNT,
			     0x00000000);
		TC_VI_REG_WT(pcdev, TEGRA_CSI_PIXEL_STREAM_A_GAP,
			     0x00000000);
		...
		TC_VI_REG_WT(pcdev, TEGRA_CSI_PIXEL_STREAM_B_WORD_COUNT,
			     0x00000000);
		TC_VI_REG_WT(pcdev, TEGRA_CSI_PIXEL_STREAM_B_GAP,
			     0x00000000);
...
struct tegra_camera_dev *pcdev = ici->priv;
	...
	if (vb2_plane_vaddr(vb, 0))
		memset(ivb2_plane_vaddr(vb, 0), 0xbd, vb2_get_plane_payload(vb, 0));
#endif
...
static int tegra_camera_stop_streaming(struct vb2_queue *q)
{
	...
	return 0; //sh_mobile_ceu_soft_reset(pcdev);
}
...
static int __devinit tegra_camera_probe(struct nvhost_device *ndev)
{
	...
	pcdev->alloc_ctx = vb2_dma_nvmap_init_ctx(NULL/*&ndev->dev*/);
