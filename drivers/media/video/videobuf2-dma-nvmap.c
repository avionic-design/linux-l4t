...
static void *vb2_dma_nvmap_alloc(void *alloc_ctx, unsigned long size)
{
	...
	ret = nvmap_mark_global(conf->nvmap_client, buf->nvmap_ref);
	if (ret < 0) {
		dev_err(conf->dev, "nvmap_mark_global failed\n");
		goto exit_unmap;
	}
...
static __u32 vb2_dma_nvmap_get_buf_handle(void *mem_priv)
{
	struct vb2_dc_buf *buf = mem_priv;

	return (__u32)buf->nvmap_ref->handle;
}
...
const struct vb2_mem_ops vb2_dma_nvmap_memops = {
	...
	.get_buf_handle = vb2_dma_nvmap_get_buf_handle,
};
