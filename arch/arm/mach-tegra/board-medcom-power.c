...
static struct tps6586x_platform_data tps_platform = {
	...
	.subdevs	= tps_devs,
	.gpio_base	= MEDCOM_GPIO_TPS6586X(0),
	.use_power_off	= true,
};

static struct i2c_board_info __initdata medcom_regulators[] = {
	{
...
static void medcom_board_suspend(int lp_state, enum suspend_stage stg)
{
	if ((lp_state == TEGRA_SUSPEND_LP1) && (stg == TEGRA_SUSPEND_BEFORE_CPU))
		tegra_console_uart_suspend();
}

static void medcom_board_resume(int lp_state, enum resume_stage stg)
{
	if ((lp_state == TEGRA_SUSPEND_LP1) && (stg == TEGRA_RESUME_AFTER_CPU))
		tegra_console_uart_resume();
}

static struct tegra_suspend_platform_data medcom_suspend_data = {
	/*
	...
	.corereq_high	= false,
	.sysclkreq_high	= true,
	.board_suspend = medcom_board_suspend,
	.board_resume = medcom_board_resume,
};

int __init medcom_suspend_init(void)
{
	tegra_init_suspend(&medcom_suspend_data);
	return 0;
}

int __init medcom_regulator_init(void)
{
	...
	i2c_register_board_info(4, medcom_regulators, 1);
